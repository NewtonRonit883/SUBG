...
warning= score_surf = text_font1.render('WARNING',False,'#D00707',)
...
player1_surf1=pygame.image.load('OP/shinchan.png')
player1_jump=pygame.image.load('OP/jump.png')
player1_surf2=pygame.image.load('OP/shinchan1.png')
player_surf=player1_surf1
player_jump=player1_jump
player_surf2=player1_surf2
player=player_surf

player2_surf1=pygame.image.load('OP/player2.png')
player2_surf1= pygame.transform.scale(player2_surf1,(113,137))
player2_jump= pygame.image.load('OP/player1.png')
player2_jump= pygame.transform.scale(player2_jump,(145,138))
player2_surf2 = pygame.image.load('OP/player3.png')
...
sound=pygame.mixer.Sound('AUDIO/Juhani Junkala [Retro Game Music Pack] Title Screen.wav')

pygame.mixer.Sound.set_volume(sound,0.05)
coin_sound=pygame.mixer.Sound('AUDIO/mixkit-winning-a-coin-video-game-2069.wav')
jump_sound=pygame.mixer.Sound('AUDIO/cartoon-jump-6462.wav')
yeah_boy=pygame.mixer.Sound('AUDIO/yeah-boy-114748.wav')
intro= pygame.mixer.Sound('AUDIO/simple-clean-logo-13775.wav')
ea_s= pygame.mixer.Sound('AUDIO/ea_sports.wav')
game1=True
while game1:
   for ok in pygame.event.get():
      if ok.type == pygame.QUIT:
         pygame.quit()
         exit()
   if loading==100:
...
while True:
   ...
   for op in pygame.event.get():
      if op.type == pygame.QUIT:
         pygame.quit()
         exit()
      elif op.type== pygame.MOUSEBUTTONDOWN:
         if background_rect.collidepoint(op.pos):
            back=True
            while back:
               screen.blit(test_surface,(0,0))
               screen.blit(detail_surf,detail_brect)
               for ronit in pygame.event.get():
                  if ronit.type == pygame.KEYDOWN:
                     if ronit.key == pygame.K_RIGHT:
                        ...
                        if test_surface==test_surface1:
                           ...
                           test_surface = test_surface2
                           detail_surf = detail_surf2
                     elif ronit.key == pygame.K_ESCAPE:
                        back=False
               pygame.display.update()
               clock.tick(60)
         elif player__rect.collidepoint(op.pos):
            play=True
            ...
            while play:
               ...
               for parul in pygame.event.get():
                  if parul.type== pygame.KEYDOWN:
                     if parul.key == pygame.K_RIGHT:
                     ...
                     elif parul.key == pygame.K_ESCAPE:
                        play = False
                  ...
                  if event.type== pygame.MOUSEBUTTONDOWN:
                     if settings_rect.collidepoint(event.pos):
                        ...
                        while blit1:
                           ...
                           for od in pygame.event.get():
                              if od.type == pygame.MOUSEBUTTONDOWN:
                                 if resume_rect.collidepoint(od.pos):
                                    blit1=False
                                    webbrowser.open('www.youtube.com')
                                    break
                                 if restart_rect1.collidepoint(od.pos):
                                    game_active = 'lol'
                                    sad1=False
                                    blit1 = False
                                    break
                                 if vol_rect.collidepoint(od.pos):
                                    if vol==vol3:
                                       vol=vol2
                                 ...
                                 if sound_rect.collidepoint(od.pos):
                                    if sound_==sound3:
                                       sound_=sound2
